...
- **操作系统版本**：Windows 11  
- **工作区路径**：`D:\ideaProject\order`
- **使用的 JDK 版本**：JDK 1.8.0_181  
- **构建工具**：Maven  
- **当前时间**：2025-10-20 17:18:05  
- **代码作者**：11276  
...
- **主框架**：Spring Boot 2.6.x（基于 `spring-boot-starter-parent:2.6.11`）
- **语言版本**：Java 1.8
- **核心依赖**：
...
| **Mapper**     | 数据库访问与持久化操作             | 使用 MyBatis Plus 提供的 CRUD 方法；避免手动写 SQL              |
| **Entity**     | 映射数据库表结构                   | 不得直接返回给前端（需转换为 DTO）；包名统一为 `entity`         |

### 接口与实现分离

- 所有接口实现类需放在接口所在包下的 `impl` 子包中。例如：`UserService` → `UserServiceImpl`。
